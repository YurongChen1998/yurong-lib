...
class PSPNet(nn.Module):
    def __init__(self, n_classes=1, sizes=(1, 2, 3, 6), psp_size=2048,
        ...
        self.decoder = nn.Sequential(
            PSPModule(psp_size, 1024),
            PSPUpsample(1024, 256, drop_rate),
            PSPUpsample(256, 64, drop_rate),
            PSPUpsample(64, 64, drop_rate),
            nn.Conv2d(64, 3, 1, 1),
            nn.Tanh()
            )
    ...
    def forward(self, x):
        ...
        p = torch.where(p>=0.5,a,b)
        

        recon_x = self.decoder(f)
