...
class JigsawDataset(data.Dataset):
    def __init__(self, test_path, img_transformer, tile_transformer, jig_classes=1000, bias_whole_image=0.7):
        #self.data_path = '/home/tzh/Documents/working_Yurong/working_05-08/Patch_WAE_MI_FINAL/images/pen' crack faulty_imprint  poke  scratch  squeeze
        #self.data_path = './images/good'
    ...
    def __retrieve_permutations(self, classes):
        all_perm = np.load('permutations_%d.npy' % (100))
...
def get_dataloader(test_path):
    ...
    train_dataset = JigsawDataset(test_path, img_transformer, tile_transformer, jig_classes=1000, bias_whole_image=0.7)
    datasets.append(train_dataset)
    dataset = ConcatDataset(datasets)
    loader = torch.utils.data.DataLoader(dataset, batch_size=1, shuffle=False, num_workers=0, pin_memory=True, drop_last=True)
    return loader
