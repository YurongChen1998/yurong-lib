...
class JigsawDataset(data.Dataset):
    def __init__(self, img_transformer, tile_transformer, jig_classes=1000, bias_whole_image=0.7):
        #self.data_path = './fig'
        self.data_path = 'images/train/normal'
    ...
    def __retrieve_permutations(self, classes):
        all_perm = np.load('permutations_%d.npy' % (0))
...
def get_dataloader():
    ...
    train_dataset = JigsawDataset(img_transformer, tile_transformer, jig_classes=1000, bias_whole_image=0.7)
    datasets.append(train_dataset)
    dataset = ConcatDataset(datasets)
    loader = torch.utils.data.DataLoader(dataset, batch_size=1, shuffle=True, num_workers=0, pin_memory=True, drop_last=True)
    return loader
    
def get_transformers():
    size = [624, 624]
    scale = [1.0, 1.0]
    img_transormer = [transforms.RandomResizedCrop((int(size[0]), int(size[1])), (scale[0], scale[1])), transforms.ColorJitter(0.2, 0.2, 0.2, 0.2)]
